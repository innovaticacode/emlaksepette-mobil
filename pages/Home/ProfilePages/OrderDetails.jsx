...
let deposit_rate = 0.04;
    ...
    let saleType = '';
    let kapora_tutari = parseFloat(Detail?.amount?.replace(',', '').replace('.', '')) / 100;
    ...
    tam_tutar_formatli = new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(tam_tutar);
    urun_fiyati_formatli = new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(urun_fiyati);
  ...
  return (
   ...
   <View style={style.orderInfo}>
        <View style={{flexDirection:'row'}}>
            ...
            <Text style={{fontSize:13,color:'grey'}}>#1000{Detail.id}</Text>
        </View>
        <View style={{flexDirection:'row'}}>
        ...
        <View style={{flexDirection:'row'}}>
            ...
            {Detail.status==0 && <Text style={{fontSize:13,color:'#BC3913',}}>1 Onay Bekliyor</Text> }
            {Detail.status==1 && <Text style={{fontSize:13,color:'#4B8F3C',}}>1 Onaylandı</Text> }
            {Detail.status==2 && <Text style={{fontSize:13,color:'#B81911',}}>Reddedildi</Text> }
        ...
        <View style={{flexDirection:'row'}}>
            ...
            {Detail.status==0 && <Text style={{fontSize:13,color:'#BC3913',}}>Onay Bekliyor</Text> }
            {Detail.status==1 && <Text style={{fontSize:13,color:'#4B8F3C',}}>Onaylandı</Text> }
            {Detail.status==2 && <Text style={{fontSize:13,color:'#B81911',}}>Reddedildi</Text> }
    
        </View>
        <View style={{flexDirection:'row'}}>
            <Text style={{fontWeight:'400',fontSize:13}}>Toplam: </Text>
            <Text style={{fontSize:13,color:'green'}}>{Detail.amount} ₺</Text>
        </View>
   </View>
   <View style={style.orderDetail}>
        ...
        <View style={{flexDirection:'row',alignItems:'center',gap:5}}>
            ...
            <Text style={{fontSize:13,color:'green'}}>{Detail?.store?.name}</Text>
        ...
        {Detail.status ==0 &&   <View style={{backgroundColor:'#FFEFCA',borderWidth:1,borderColor:'#FFCC85',padding:5,borderRadius:5,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:10}}>
                  
                  <Text style={{color:'#BC3913',textAlign:'center',fontSize:12}}>Onay Bekliyor</Text>
                  <FeatherIcon name="clock" color={'#BC3913'}/>
              </View>}
              {Detail.status ==1 &&   <View style={{backgroundColor:'#D9F9D0',borderWidth:1,borderColor:'#BEE8B4',padding:5,borderRadius:5,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:10}}>
                  <Text style={{color:'#4B8F3C',textAlign:'center',fontSize:12}}> Ödeme  Onaylandı</Text>
                  <FeatherIcon name="check" color={'#4B8F3C'} size={16}/>
              </View>}
              {Detail.status == 2 &&   <View style={{backgroundColor:'#FFE0DB',borderWidth:1,borderColor:'#FABCB3',paddingLeft:6,paddingRight:6,padding:4,borderRadius:5,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:6,overflow:'hidden'}}>
            ...
            <View style={style.OrderPost}>
                    ...
                    <ImageBackground source={{uri:parsedData?.item?.image}} style={{width:'100%',height:'100%'}} resizeMode='cover'/>
                    </View>
                    <View style={style.caption}>
                        <Text style={{fontWeight:'500'}}>{parsedData?.item?.title}</Text>
                        {parsedData.type== 'project'    &&  <Text style={{fontWeight:'500'}}>{parsedData?.item?.housing} No'lu Konut</Text> }
                    
                        
                        <View style={{paddingTop:5,backgroundColor:'#DAFBD0',width:'50%',padding:5,borderRadius:5}}>
                            <Text style={{textAlign:'center',color:'#3E8330',fontWeight:'bold'}}>{Detail?.amount }</Text>
                        </View>
                    </View>
   ...
   <View style={[style.PersonalInfoArea]}>
    <Text style={{textAlign:'center',fontSize:15,color:'#333',fontWeight:'500'}}>Alıcı Bilgileri</Text>
    ...
    <View style={[style.PersonalInfoArea]}>
    <Text style={{textAlign:'center',fontSize:15,color:'#333',fontWeight:'500'}}>Satıcı Bilgileri</Text>
        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
    ...
    <View style={style.paymentArea}>
        <Text style={{textAlign:'center',fontSize:15,color:'#333',fontWeight:'500'}}>Özet</Text>
        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
        <View>
    ...
    </View>
    {
        ...
        <View>
        <TouchableOpacity style={{backgroundColor:'#E54242',padding:13,borderRadius:5}}>
            <Text style={{color:'#ffffff',textAlign:'center',fontWeight:'500'}}>İade Talebi Oluştur</Text>
        </TouchableOpacity>
       </View>
    }
        {
        ...
        <TouchableOpacity style={{backgroundColor:'#E54242',padding:13,borderRadius:5}}>
            <Text style={{color:'#ffffff',textAlign:'center',fontWeight:'500'}}>Yükle</Text>
        </TouchableOpacity>
       </View>
    }
