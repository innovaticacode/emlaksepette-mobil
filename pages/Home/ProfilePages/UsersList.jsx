...
export default function UsersList() {
    ...
    try {
      if (user.access_token) {
        const response = await axios.get(
          "http://192.168.1.102:8000/api/institutional/users",
          {
            headers: {
    ...
    try {
      if (user.access_token) {
        const response = await axios.delete(
          `http://192.168.1.102:8000/api/institutional/users/${selectedUser}`,
          {
            headers: {
              Authorization: `Bearer ${user.access_token}`,
            },
  ...
  const deleteAllUsers = async () => {
    ...
    try {
      const response = await axios.delete(
        "http://192.168.1.102:8000/api/institutional/sub-users",
        {
          data: data,
          headers: {
            Authorization: `Bearer ${user.access_token}`,
