...
export default function EditCollectionPost({item}) {
    ...
    if(item.item_type == 1){
        ...
        if(item.housing.housing_type_data){
            var itemData = {
                price : addDotEveryThreeDigits(JSON.parse(item.housing.housing_type_data).price),
                advertise_title : item.housing.title,
                image : JSON.parse(item.housing.housing_type_data).image,
                earning : parseInt(JSON.parse( item.housing.housing_type_data).price) / 100
            }
        }else{
            var itemData = {
                price : 0,
  ...
  return (
    ...
    <View style={{flexDirection:'row',gap:4}}>
        <View style={{flex:0.5/2,padding:1,height:90}}>

        </View>
        <View style={{flex:1.1/2,padding:1}}>
                <View style={{flex:1/2}}>
                    <Text style={{fontSize:13}} numberOfLines={2}>{itemData.advertise_title}</Text>
                </View>
                <View style={{flex:1/2,flexDirection:'row',justifyContent:'space-between'}}>
                  <View style={{width:'50%',gap:4}}>
                  <Text style={{fontSize:13,textAlign:'center',color:'#333'}} >Fiyat</Text>
                    <Text style={{textAlign:'center',fontSize:13,color:'#298A1D'}}>{itemData.price}₺</Text>
                  </View>

                  <View style={{width:'50%',gap:4}}>
                    <Text style={{fontSize:13,textAlign:'center',color:'#333'}}>Kazanç</Text>
                    <Text style={{textAlign:'center',fontSize:13,color:'#32B31F'}}>{addDotEveryThreeDigits(itemData.earning)}₺</Text>
                  </View>

                </View>
        </View>
        <View style={{flex:0.4/2,alignItems:'center',justifyContent:'center'}}>
            <TouchableOpacity style={{backgroundColor:'#E54242',width:'100%',padding:8,borderRadius:6}}>
