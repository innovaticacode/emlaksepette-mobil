...
export default function PostDetail() {
  ...
  const copyToClipboard = async () => {
    await Clipboard.setStringAsync(
      `http://192.168.18.32:8000/ilan/${data?.housing?.step1_slug}-${data?.housing?.step2_slug}-${data?.housing?.slug}/2000${data?.housing?.id}/detay`
    );
    alert("Metin kopyalandı!");
  };
  const shareLinkOnWhatsApp = () => {
    const url = `http://192.168.18.32:8000/ilan/${data?.housing?.step1_slug}-${data?.housing?.step2_slug}-${data?.housing?.slug}/2000${data?.housing?.id}/detay`;
  ...
  const onShare = async () => {
    try {
      const result = await Share.share({
        message: `http://192.168.18.32:8000/ilan/${data?.housing?.step1_slug}-${data?.housing?.step2_slug}-${data?.housing?.slug}/2000${data?.housing?.id}/detay`,
      });
  ...
  const addFavorites = () => {
    if (user.access_token) {
      ...
      axios
        .post(
          `http://192.168.18.32:8000/api/add_housing_to_favorites/${data?.housing?.id}`,
          {},
          config
        )
        .then((res) => {
...
console.log(user.access_token + user.name)
  return (
    <>
      <AlertNotificationRoot>
        {loading ? (
