...
const CreateReservation = () => {
  ...
  const getBookedDates = (reservations) => {
    ...
    reservations?.forEach((reservation) => {
      ...
      if (isValid(startDate) && isValid(endDate)) {
        ...
        dates = dates.concat(
          interval.map((date) => format(date, "yyyy-MM-dd"))
        );
      }
    });
    return dates;
  ...
  const totalCost =
    selectedStartDate && selectedEndDate && data?.housing?.housing_type_data
      ? JSON.parse(data?.housing?.housing_type_data)["daily_rent"] * totalNights
      : 0;
  ...
  const totalPrice = formatCurrency(
    checked ? halfTotalCost + 1000 : halfTotalCost
  );
  ...
  const saveAllStates = async () => {
    try {
      ...
      navigation.navigate("PaymentScreenForReserve", {
  ...
  console.log(
    data &&
      ...
      JSON.parse(data?.housing?.housing_type_data)["daily_rent"] + "dfskdf"
  );
  return (
    <AlertNotificationRoot>
      <ScrollView
        ...
        <View style={styles.dateContainer}>
          ...
          >
            ...
            <Text style={{ fontSize: 13 }}>
              Param Güvende seçeneğini işaretlerseniz rezervasyon iptal
              durumunda paranızın iadesinde kesinti olmayacaktır.
            </Text>
          </View>
        </View>
        ...
        <View style={[styles.dateContainer, { gap: 10 }]}>
          ...
          <View style={{ gap: 20 }}>
            ...
            >
              ...
              <View style={{ width: "50%", alignItems: "flex-end" }}>
                <Text style={styles.DetailTitle}>{totalNights} x Gece</Text>
              </View>
            </View>
            <View
              style={{ flexDirection: "row", justifyContent: "space-between" }}
            >
              ...
              <View style={{ width: "50%", alignItems: "flex-end" }}>
                <Text style={styles.DetailTitle}>{formattedTotalCost}</Text>
              </View>
            </View>
            <View
              style={{ flexDirection: "row", justifyContent: "space-between" }}
