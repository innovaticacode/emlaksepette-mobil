...
export default function CreditCardScreen({ getError,CompeletePayment, creditCartData, setCreditCartData , errors }) {
  const months = [
    { label: 'Ocak', value: 1 },
    { label: 'Şubat', value: 2 },
    { label: 'Mart', value: 3 },
  ...
  const [creditCartWarningColor,setCreditCartWarningColor] = useState("#cc3300")
    ...
    <View>
      ...
      <View style={{ paddingTop: 10, gap: 10 }}>
        <View style={{ gap: 6 }}>
          ...
          <AbsoluteErrorInput warningColor={creditCartWarningColor} warningText={creditCartWarningText} show={true}/>
          <TextInput style={{...styles.Input,borderColor : getError('credit_cart_number')}} value={creditCartData.credit_cart_number} 
            onChangeText={(value) => { 
              const numericValue = value.replace(/[^0-9]/g, ''); 
                if(value.replace(' ','').replace(' ','').replace(' ','').length > 16){
                  setCreditCartWarningText("Maksimum karakter sayısına ulaştınız");
                  setCreditCartWarningColor('#cc3300')
                  setCreditCartData({ ...creditCartData, credit_cart_number: formatNumber(value).substring(0,19) });
                  handleCreditCardNumberChange(formatNumber(value).substring(0,19))
                  ...
                  if(numericValue == value.replace(' ','').replace(' ','').replace(' ','')){
                    if(value.replace(' ','').replace(' ','').replace(' ','').length == 16){
                      setCreditCartWarningText("Başarılı")
                      setCreditCartWarningColor("green")
                      setCreditCartData({ ...creditCartData, credit_cart_number: formatNumber(value) });
                    ...
                    setCreditCartData({ ...creditCartData, credit_cart_number: formatNumber(numericValue) });
        ...
        <View style={{ gap: 6 }}>
          ...
          <View style={{marginTop:10}}>
            <AbsoluteErrorInput style={{top : -11}} warningColor={nameWarningColor} warningText={nameWarningText} show={true}/>
            <TextInput style={{...styles.Input,borderColor : getError('name')}} value={NameOfCardUser} 
              onChangeText={(value) => {
                if(value.length > 40){
                  setNameWarningText("Maksimum karakter sayısına ulaştınız")
                  setNameWarningColor("#cc3300")
                  setCreditCartData({ ...creditCartData, name: value.substring(0,40) });
                  ...
                  if(value.length < 5){
                    ...
                    if(value.split(' ').length < 2){
                      setNameWarningText("Adınız ve soyadınız arasına boşluk koyarak yazınız")
                      setNameWarningColor("#cc3300")
                      setNameOfCardUser(value)
                    }else{
                      if(value.split(' ')[1].length > 0){
                        if(value.split(' ')[1].length > 1){
                          if(value.split(' ')[0].length > 2){
                            ...
                            setNameWarningText("Adınız minimum 3 karakter olmalıdır")
                            setNameWarningColor("#cc3300")
                            setNameOfCardUser(value)
                          }
                        }else{
                          setNameWarningText("Soyadınız minimum 2 karakter olmalıdır")
                          setNameWarningColor("#cc3300")
                          setNameOfCardUser(value)
                        }
                      }else{
        ...
        <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between',marginTop:10,gap:10}}>
          <View style={{flex : 1}}>
            <AbsoluteErrorInput style={{top:-11}} warningColor={monthWarningColor} warningText={monthWarningText} show={true}/>
            <RNPickerSelect doneText="Tamam"
              ...
              onValueChange={(value) => { 
                if(value){
                  ...
                  setmonth(value); setCreditCartData({ ...creditCartData, exp_month: value }) 
                }else{
              ...
              style={{inputIOS : {...pickerSelectStyles.inputIOS,borderColor : getError('month')},inputAndroid : {...pickerSelectStyles.inputAndroid,borderColor : getError('month')}}}
              value={creditCartData.exp_month}
              items={months}
            />
          </View>
          <View style={{flex : 1}}>
            <AbsoluteErrorInput style={{top:-11}} warningColor={yearWarningColor} warningText={yearWarningText} show={true}/>
            <RNPickerSelect doneText="Tamam"
              placeholder={{
                label: "Seçiniz",
                value: null,
              ...
              onValueChange={(value) => { 
                if(value){
                  ...
                  setYear(value); setCreditCartData({ ...creditCartData, exp_year: value })
                }else{
                  setYear(value); setCreditCartData({ ...creditCartData, exp_year: value })
              ...
              style={{inputIOS : {...pickerSelectStyles.inputIOS,borderColor : getError('year')},inputAndroid : {...pickerSelectStyles.inputAndroid,borderColor : getError('year')}}}
