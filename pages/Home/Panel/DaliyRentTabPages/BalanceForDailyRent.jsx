...
export default function BalanceForDailyRent() {
  ...
  const filterData = searchTerm
    ? SummaryRedux?.rental_summary?.sale_prices_sum?.approved.filter(item => {
        const title = item.housing.title.toLowerCase();
        const description = `2000${item.housing.id}`;
        const term = searchTerm.toLowerCase();
        return title.includes(term) || description.includes(term);
      })
    : SummaryRedux?.rental_summary?.sale_prices_sum?.approved;
  return (
