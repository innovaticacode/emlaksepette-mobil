...
export default function BalanceForProject() {
  ...
  const filterData = searchTerm
    ? SummaryRedux?.project_summary?.sale_prices_sum?.approved.filter(
        (item) => {
  ...
  return (
    ...
    >
      ...
      />
      {SummaryRedux?.project_summary?.sale_prices_sum?.approved.length !==
        0 && (
        <View style={{}}>
      ...
      {searchTerm && filterData.length == 0 ? (
        ...
        filterData?.map((item, index) => (
          <AdvertComponent
            ...
            ımage={
              frontEndUriBase +
              item?.project?.image?.replace("public/", "storage/")
            }
            price={item?.price}
          />
        ))
