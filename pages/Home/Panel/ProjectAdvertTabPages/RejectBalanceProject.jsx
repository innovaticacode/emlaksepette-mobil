...
export default function RejectBalanceProject() {
  const SummaryRedux = useSelector(
    (state) => state.summary.dataSummary
  );
  const RejectedTotal =
  SummaryRedux?.project_summary?.sale_prices_sum?.rejected?.reduce(
    (accumulator, item) => {
  ...
  const filterData = searchTerm
    ? SummaryRedux?.project_summary?.sale_prices_sum?.rejected.filter(item => {
        const title = item.project.project_title.toLowerCase();
        const description =`1000${item.project.id}`;
        const term = searchTerm.toLowerCase();
        return title.includes(term) || description.includes(term);
      })
    : SummaryRedux?.project_summary?.sale_prices_sum?.rejected;
