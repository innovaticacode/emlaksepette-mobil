...
export default function PaymentPlanModal({
  ...
  useEffect(() => {
    ...
    if (data && RoomOrder !== null) {
      ...
      for (
        let _index = 0;
        _index < data["pay-dec-count" + RoomOrder];
        _index++
      ) {
        const priceString = addDotEveryThreeDigits(
          data["share_sale[]"] !== "[]"
            ? (
                data[`pay_desc_price${RoomOrder}` + _index] /
        ...
        const date = new Date(data["pay_desc_date" + RoomOrder + _index]);
  ...
  const [MonthlyPriceWithoutShare, setMonthlyPriceWithoutShare] =
    useState(null);
          ...
          >
              ...
              <View style={styles.ongorulenView}>
                ...
                <View>
                  <Text style={styles.text}>
                    %{addDotEveryThreeDigits(data["projected_earnings[]"])}
                  </Text>
                </View>
              </View>
            )}
            {data["projected_earnings[]"] && data["ong_kira[]"] && (
              <View style={styles.ongorulenView}>
                <Text style={styles.text}>Aylık Kira Getirisi</Text>
                <View>
                  <Text style={styles.text}>
                    {addDotEveryThreeDigits(data["ong_kira[]"])}₺
                  </Text>
                </View>
              </View>
            )}
            <View style={{ gap: 10 }}>
              <SettingsItem
                info={"Peşin Fiyat"}
                numbers={
                  data["share_sale[]"] != "[]" && data["number_of_shares[]"]
                    ? addDotEveryThreeDigits(
                        (
                          parseInt(data["price[]"].replace(".", "")) /
                          parseInt(data["number_of_shares[]"].replace(".", ""))
                        ).toFixed(0)
                      )
                    : addDotEveryThreeDigits(data["price[]"]) + " ₺"
                }
              />
              {data &&
                RoomOrder !== null &&
                data["payment-plan[]"] &&
                JSON.parse(data["payment-plan[]"]) &&
                JSON.parse(data["payment-plan[]"]).includes("taksitli") && (
                  <>
                    {data["installments-price[]"] && (
                      <SettingsItem
                        border={"0"}
                        info={
                          data["installments[]"] + " " + "Ay Taksitli Fiyat"
                        }
                        numbers={
                          data["share_sale[]"] !== "[]" &&
                          data["number_of_shares[]"]
                            ...
                            : addDotEveryThreeDigits(
                                Math.round(data["installments-price[]"])
                              ) + " ₺"
                        }
                      />
                    )}
                    ...
                    <SettingsItem
                      ...
                      numbers={
                        ...
                        data["number_of_shares[]"]
                          ? addDotEveryThreeDigits(
                              Math.round(
                                data["advance[]"] / data["number_of_shares[]"]
                              )
                            ) + "₺"
                          : addDotEveryThreeDigits(
                              Math.round(data["advance[]"])
                            ) + " ₺"
                      }
                    />
                    {data["installments-price[]"] && (
                      <SettingsItem
                        ...
                        numbers={
                          ...
                          data["number_of_shares[]"]
                            ...
                            : addDotEveryThreeDigits(
                                (
                                  (parseInt(data["installments-price[]"]) -
                                    (parseInt(data["advance[]"]) +
                                      parseInt(totalPrice))) /
                                  parseInt(data["installments[]"])
                                ).toFixed(0)
            ...
            <View style={{ gap: 10, alignItems: "center" }}>
              <View>
                ...
                >
                  Bu ilan satış noktalarımızda %4 indirimli en yakın satış
                  noktası için tıklayın.
                </Text>
              </View>
              <View style={{ width: "60%" }}>
            ...
            >
              <View style={{}}>
                ...
                >
                  Şu anda Ödenecek Kapora Tutarı
                </Text>
              </View>
              <View>
                <Text
            ...
            <View>
              ...
              >
                ‘‘Şimdi Alın veya Kiralayın, kaporanız emlaksepette.com’da
                güvence altında!’’
              </Text>
            </View>
          </ScrollView>
